...
module.exports = {
  async login(req, res) {
    // FIXME: make dynamic
    const scope = 'links:read,links:write,commands,chat:write:bot,team:read';
  ...
  async callback(req, res) {
    ...
    let access;
    try {
      access = await client.oauth.access({
    ...
    let auth_test;
    try {
      auth_test = await client.auth.test({
        token: access.access_token,
      });

      await workspace.client.chat.postMessage({
        channel: auth_test.user_id,
        ...new SlackInstalled().toJSON(),
      });
    } catch (e) {
      req.log.debug(auth_test, 'Failed to send welcome message to user');
    }
